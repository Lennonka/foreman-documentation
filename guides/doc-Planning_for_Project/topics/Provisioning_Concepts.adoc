...
To provision machines by using PXE booting, ensure that you meet the following requirements:
...
* Ensure that DHCP with the correct subnet is enabled by using the {Project} installer.
* Enable TFTP by using the {Project} installer.

[[http-booting]]
=== HTTP booting
You can use HTTP booting to boot systems over a network by using HTTP.
...
* The TCP port {smartproxy_port} is open for the client to download the bootloader from the {SmartProxy} by using the HTTPS protocol.
...
When {Project} is installed on {EL} by using `{foreman-installer}`, grub2 and shim bootloaders that are signed by Red Hat are deployed into the TFTP and HTTP UEFI Boot directory.
PXE loader options named "SecureBoot" configure hosts to load `shim.efi`.

On Debian and Ubuntu operating systems, the grub2 bootloader is created by using the `grub2-mkimage` unsigned.
