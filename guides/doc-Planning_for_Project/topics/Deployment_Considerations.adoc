...
This allows system administrators the ability to manage Example Corporation 1's systems by using {Project} completely independently of Example Corporation 2's organizations (R&D, Operations, and Engineering).
...
This is not necessary for a connected {ProjectServer} that is subscribed by using the Subscription Manager utility on the base system.
