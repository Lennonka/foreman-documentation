...
This means TFTP boot will take place by using this interface, or in case of image based provisioning, the script to complete the provisioning will be executed through this interface.
ifdef::orcharhino[]
...
ifndef::orcharhino[]
For the list of values, see {RHELDocsBaseURL}8/html/performing_a_standard_rhel_8_installation/graphical-installation_graphical-installation#system_purposes-adoc_connect-to-red-hat[Introduction to System Purpose] in _Performing a standard RHEL 8 installation_.
endif::[]
+
* `syspurpose_role`
* `syspurpose_sla`
...
Other properties of the primary network interface can be inherited from the host group or set by using the `--subnet`, and `--domain` parameters.
You can set additional interfaces by using the `--interface` option, which accepts a list of key-value pairs.
