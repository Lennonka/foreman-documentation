...
To create hosts by using image-based provisioning, you must add information about the image, such as access details and the image location, to your {ProjectServer}.
...
ifdef::gce-provisioning[Specify a user other than `root`, because the `root` user cannot connect to a GCE instance by using SSH keys. The username must begin with a letter and consist of lowercase letters and numbers.]
...
With the `--username` option, specify a user other than `root`, because the `root` user cannot connect to a GCE instance by using SSH keys.
