...
This behavior can be turned off by using `{foreman-installer} --foreman-proxy-dhcp-ping-free-ip=false`.
...
| 7911 | TCP | DHCP, OMAPI | DHCP Server| DHCP | The DHCP target is configured by using `--foreman-proxy-dhcp-server` and defaults to localhost
