...
When you create a host on Microsoft Azure by using this compute profile, these settings are automatically populated.
...
* To authenticate by using a password, enter a password in the *Password* field.
* To authenticate by using an SSH key, enter an SSH key in the *SSH Key* field.
