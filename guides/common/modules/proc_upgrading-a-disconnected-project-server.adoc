...
* If you customized configuration files, either manually or by using a tool such as Hiera, these changes are overwritten when you enter the `{foreman-maintain}` command during upgrading or updating.
...
Select all other organizations in turn by using the organization setting menu and repeat this action as required.
...
. Optional: If you made manual edits to DNS or DHCP configuration files, check and restore any changes required to the DNS and DHCP configuration files by using the backups that you made.
