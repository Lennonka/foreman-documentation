...
. Enable the Puppet agent by using a host parameter in global parameters or a host group.
...
. Specify configuration for the Puppet agent by using the following host parameters in global parameters or a host group:
...
ifdef::katello,orcharhino,satellite[]
. Navigate to *Hosts* > *Register Host* and register your host by using an appropriate activation key.
endif::[]
ifndef::katello,orcharhino,satellite[]
