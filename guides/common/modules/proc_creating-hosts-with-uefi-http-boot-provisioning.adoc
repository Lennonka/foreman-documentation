...
You can provision hosts from {Project} by using the UEFI HTTP Boot.
...
----
. Ensure the network interface options are set by using the `hammer host interface update` command:
+
[options="nowrap" subs="+quotes"]
----
