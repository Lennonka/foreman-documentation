[id="Deploying_a_Policy_in_a_Host_Group_by_Using_Ansible_{context}"]
= Deploying a policy in a host group by using Ansible

After you deploy a compliance policy in a host group by using Ansible, the Ansible role installs the SCAP client and configures OpenSCAP scans on the hosts according to the selected compliance policy.
