...
Use this procedure to add a remote console connection by using VNC for a host on Proxmox.
...
+
To enable a remote console connection for multiple hosts, open a range of ports by using `firewall-cmd --add-port=5900-_My_Upper_Port_Range_Limit_/tcp --permanent`.
