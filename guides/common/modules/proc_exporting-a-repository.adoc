...
.Procedure
. Export a repository by using the product name and repository name:
+
...
If you want to split the exported archive into chunks, export your repository by using the `--chunk-size-gb` argument to limit the size by an integer value in gigabytes, for example `---chunk-size-gb=_2_`.
====
