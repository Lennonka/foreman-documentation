[id="Deploying_a_Policy_on_a_Host_by_Using_Puppet_{context}"]
= Deploying a policy on a host by using Puppet

After you deploy a compliance policy on a host by using Puppet, the Puppet agent installs the SCAP client and configures OpenSCAP scans on the host on the next Puppet run according to the selected compliance policy.
