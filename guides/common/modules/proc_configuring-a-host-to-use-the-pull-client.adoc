...
For {SmartProxies} configured to use `pull-mqtt` mode, hosts can subscribe to remote jobs by using the remote execution pull client.
...
* The host can communicate with its {SmartProxy} over MQTT by using port `1883`.
