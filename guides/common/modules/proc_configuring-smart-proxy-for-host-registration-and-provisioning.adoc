...
Use this procedure to configure {SmartProxy} so that you can register and provision hosts by using your {SmartProxyServer} instead of your {ProjectServer}.
...
.Verification
. List the current trusted proxies by using the full help of {Project} installer:
+
[options="nowrap" subs="+quotes,attributes"]
----
