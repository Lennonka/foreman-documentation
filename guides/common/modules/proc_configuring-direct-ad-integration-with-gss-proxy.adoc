...
. Retrieve the Kerberos ticket of the LDAP user, by using the following command:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# kinit ldapuser
----
. View the Kerberos ticket, by using the following command:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# klist
----
. View output from successful SSO-based authentication, by using the following command:
+
