...
. Write the ISO to a USB storage device by using the `dd` utility or `livecd-tools` if required.
...
+
When the installation completes, the host also registers to {ProjectServer} by using the activation key and installs the necessary configuration and management tools from the *{project-client-name}* repository.
endif::[]

[id="cli-creating-hosts-with-pxe-less-provisioning_{context}"]
.CLI procedure
. Create the host by using the `hammer host create` command.
+
...
----
. Ensure that your network interface options are set by using the `hammer host interface update` command.
+
[options="nowrap" subs="+quotes"]
----
...
--provision true
----
. Download the boot disk from {ProjectServer} by using the `hammer bootdisk` command:
+
ifndef::satellite[]
* For *Host image*:
+
