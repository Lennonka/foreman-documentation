You can register hosts with {Project} by using global registration in the {ProjectWebUI}, Hammer CLI, or the {Project} API.
...
. Connect to your host by using SSH and run the registration command.
ifdef::satellite[]
...
.CLI procedure
. Generate the host registration command by using the Hammer CLI:
+
...
.API procedure
. Generate the host registration command by using the {Project} API:
+
ifndef::katello,satellite,orcharhino[]
[options="nowrap" subs="+quotes,attributes"]
----
