...
* A backup of your {ProjectServer} that you make by using the `{foreman-maintain} backup` script.
...
* Clone by using backup with Pulp data
* Clone by using backup without Pulp data and copy `/var/lib/pulp` manually from the source server.
...
After you perform an upgrade by using the following chapters, you can safely decommission the source server.
