...
Note that you cannot change host parameters as existing hosts are not redeployed but only reconfigured by using Ansible.
...
+
You can refer to any {Project} parameter by using `PARAM[_name_of_your_parameter_]`, for example, based on an operating system or a host group.
