...
. Transfer the `.iso` image, the signature, and the checksum from your local machine to {ProjectServer} by using `scp`:
+
...
----
. On your {ProjectServer}, verify the integrity and authenticity of the `.iso` image by using GPG public keys:
+
[options="nowrap", subs="+quotes,verbatim,attributes"]
----
# cd /tmp
