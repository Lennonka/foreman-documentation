[id="Browsing_Repository_Content_by_Using_an_Organization_Debug_Certificate_{context}"]
= Browsing repository content by using an organization debug certificate

You can view an organization's repository content by using a web browser or by using the API if you have a debug certificate for that organization.
...
.For Firefox users
. Convert the certificate into the PKCS12 format by using the following command:
+
[subs="+quotes"]
----
