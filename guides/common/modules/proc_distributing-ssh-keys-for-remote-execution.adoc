...
For {SmartProxies} in `ssh` mode, remote execution connections are authenticated by using SSH.
...
. For new {Project} hosts, you can deploy SSH keys to {Project} hosts during registration by using the global registration template.
