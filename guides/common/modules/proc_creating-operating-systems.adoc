...
You can also add custom operating systems by using the following procedure.
endif::[]
ifdef::foreman-deb,orcharhino[]
You can add operating systems by using the following procedure.
endif::[]
...
.CLI procedure
* Create the operating system by using the `hammer os create` command:
+
