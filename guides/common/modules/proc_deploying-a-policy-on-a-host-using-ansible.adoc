[id="Deploying_a_Policy_on_a_Host_by_Using_Ansible_{context}"]
= Deploying a policy on a host by using Ansible

After you deploy a compliance policy on a host by using Ansible, the Ansible role installs the SCAP client and configures OpenSCAP scans on the host according to the selected compliance policy.
