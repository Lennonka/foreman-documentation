[id="Propagating_SCAP_Content_by_using_Ansible_Deployment_{context}"]
= Propagating SCAP content by using Ansible deployment
...
. In the *Specify Matchers* area, remove all matchers that override the default value.
. Click *Submit*.
. Continue with deploying a compliance policy by using Ansible.
For more information, see:
* {ManagingSecurityDocURL}Deploying_a_Policy_in_a_Host_Group_by_Using_Ansible_security-compliance[Deploying a Policy in a Host Group Using Ansible] in _{ManagingSecurityDocTitle}_
* {ManagingSecurityDocURL}Deploying_a_Policy_on_a_Host_by_Using_Ansible_security-compliance[Deploying a Policy on a Host Using Ansible] in _{ManagingSecurityDocTitle}_
