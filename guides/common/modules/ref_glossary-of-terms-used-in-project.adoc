...
When you register a host by using an activation key, certain content from {Project} is provided to the host.
...
For more information about automating {Project} by using {Project} Ansible collection, see {ManagingConfigurationsAnsibleDocURL}Managing_Project_with_Ansible_Collections_admin[Managing {Project} with Ansible collections] in _{AdministeringDocTitle}_.
...
A compliance policy requires the setting of a specific XCCDF profile from a SCAP content and, optionally, a tailoring file.
...
Content is synchronized into the Library and then promoted into lifecycle environments by using content views so that they can be consumed by hosts.
...
Once a content view is published, it can be promoted through the lifecycle environment path, or modified by using incremental upgrades.
...
For more information, see https://atixservice.zendesk.com/hc/de/articles/9464966748444[Registering orcharhino Server to OCC] and https://atixservice.zendesk.com/hc/de/articles/7041824258460[Adding orcharhino Clients manually] in the _ATIX Service Portal_.
...
ifndef::satellite[]
If you manage SUSE content by using SCC Manager plugin, {Project} automatically groups SUSE content within products.
...
The files contain code to define a set of necessary resources, such as packages, services, files, users and groups, and so on, by using a set of key-value pairs for their attributes.
+
...
[[Virtualization]]
Virtualization::
Virtualization describes the process of running multiple operating systems with various applications on a single hardware host by using hypervisors like
ifdef::satellite[]
VMware or libvirt.
endif::[]
ifndef::satellite[]
